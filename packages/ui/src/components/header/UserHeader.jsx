  ...
  return (
    <header className="h-16 border-b bg-white flex items-center justify-between px-4 md:px-6 sticky top-0 z-10 gap-2 md:gap-4">
      ...
      <div className="flex items-center">
        ...
        <div className="hidden  lg:flex items-center pt-1 pb-1 gap-6">
          <img
          ...
          <ul className="flex justify-around font-bold text-sm text-gray-500 uppercase">
            {userNavLink.map((link) => (
              <NavLink
                key={link.label}
                to={link.path}
                className=" text-gray-500 hover:text-primary rounded-lg px-3 py-1"
              >
                {link.label}
              </NavLink>
            ))}
      ...
      <div className="flex items-center gap-1 md:gap-4 shrink-0">
        ...
        <div className="relative" ref={notifRef}>
          ...
          {isNotifOpen && (
            <div className="absolute right-0 mt-2 w-72 md:w-80 bg-white border border-gray-200 rounded-xl shadow-xl z-50">
              ...
              <div className="max-h-80 overflow-y-auto">
                {usernotificationsSchema.map((n) => (
                  <button
                    key={n.id}
                    className="w-full cursor-pointer flex items-start gap-3 p-2 hover:bg-gray-50 border-b border-gray-50 last:border-0 text-left"
                  >
                    <div className={`p-2 rounded-lg shrink-0 ${n.bg}`}>
                      <n.icon size={16} className={n.iconColor} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm relative font-semibold truncate">
                        {n.title}
                        <span className="text-red-400 rounded-full h-4 w-4 absolute text-[10px] -top-1">
                          10
                        </span>
                      </p>
