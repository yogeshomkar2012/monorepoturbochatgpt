...
const UserSideBar = ({ NavLink, socialLinksData, data, className = '' }) => {
  ...
  const baseClassname = `${className} w-64 h-screen  sticky top-0 bg-primary text-text-white grid grid-rows-[auto_1fr_auto]`;
  return (
    <aside className={`${baseClassname}`}>
      ...
      <div className="border-t border-white/10 p-4 space-y-4">
        <button className="w-full cursor-pointer flex items-center justify-center gap-2 py-2 bg-red-500 hover:bg-red-600 rounded-md">
          <LogOut size={18} />
          <span>Logout</span>
        </button>
